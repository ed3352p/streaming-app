...
export default function EditSeries() {
  ...
  useEffect(() => {
    ...
    const found = allSeries.find(s => s.id === parseInt(id));
    ...
    if (found) {
      ...
      setSeasons(found.seasons || [{ number: 1, episodes: [{ number: 1, title: '', videoUrl: '' }] }]);
    } else {
      setNotFound(true);
    }
  }, [id, navigate]);
  ...
  const handleSubmit = (e) => {
    ...
    const allSeries = JSON.parse(localStorage.getItem('series') || '[]');
    const index = allSeries.findIndex(s => s.id === parseInt(id));
  ...
  if (notFound) {
    return (
      <div className="container" style={{textAlign: 'center', paddingTop: '100px'}}>
        ...
        <p style={{color: '#64748b', marginTop: '20px'}}>Cette série n'existe pas.</p>
        <a href="/admin/series" className="btn" style={{marginTop: '20px'}}>Retour aux séries</a>
      </div>
    );
  }
